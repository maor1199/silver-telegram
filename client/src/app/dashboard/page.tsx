    ...
    <div className={cn("rounded-2xl border p-5 transition-shadow hover:shadow-md", cfg.bg)}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          <div className={cn("mt-0.5 rounded-lg p-1.5 flex-shrink-0", alert.severity === "critical" ? "bg-red-100" : alert.severity === "high" ? "bg-orange-100" : "bg-yellow-100")}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="mt-1.5 text-xs text-muted-foreground leading-relaxed">{alert.description}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          {alert.dataUsed.slice(0, 2).map((d, i) => (
            <span key={i} className="rounded-full bg-background border border-border px-2 py-0.5 text-[10px] text-muted-foreground">{d}</span>
          ))}
        </div>
      </div>
    </div>
...
export default function CommandCenterPage() {
  ...
  const hcfg = healthConfig(health.healthStatus)

  const criticalAlerts = alerts.filter(a => a.severity === "critical")
  const highAlerts = alerts.filter(a => a.severity === "high")
  const mediumAlerts = alerts.filter(a => a.severity === "medium")

  const urgentActions = alerts
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {health.totalAlerts === 0
                  ...
                  : `${health.totalAlerts} active risk${health.totalAlerts !== 1 ? "s" : ""} across ${health.activeSkus} SKUs need attention.`}
              </p>
            </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="mb-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {criticalAlerts.length > 0 && (
                    <span className="rounded-full bg-red-100 text-red-700 border border-red-200 px-2.5 py-0.5 text-xs font-bold">
                      {criticalAlerts.length} Critical
                    </span>
                  )}
                  {highAlerts.length > 0 && (
                    <span className="rounded-full bg-orange-100 text-orange-700 border border-orange-200 px-2.5 py-0.5 text-xs font-bold">
                      {highAlerts.length} High
                    </span>
                  )}
                  {mediumAlerts.length > 0 && (
                    <span className="rounded-full bg-yellow-100 text-yellow-700 border border-yellow-200 px-2.5 py-0.5 text-xs font-bold">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-border bg-card p-5">
                ...
                <div className="space-y-2">
                  {MOCK_SKUS.filter(s => s.status === "active").map(sku => {
                    const skuAlerts = alerts.filter(a => a.skuId === sku.id)
                    const worst = skuAlerts[0]
                    const statusColor = worst?.severity === "critical" ? "text-red-600" :
                      worst?.severity === "high" ? "text-orange-600" :
                      worst?.severity === "medium" ? "text-yellow-600" : "text-green-600"
                    const dot = worst?.severity === "critical" ? "bg-red-500 animate-pulse" :
                      worst?.severity === "high" ? "bg-orange-500" :
                      worst?.severity === "medium" ? "bg-yellow-500" : "bg-green-500"
