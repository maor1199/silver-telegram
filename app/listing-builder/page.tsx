...
/* ── Config ── */
const NGROK_URL = "https://unresuscitable-unskirted-shaniqua.ngrok-free.dev/api/generate-listing"
...
/** Safely extract display text from a bullet point (object or string) */
function bulletToText(b: unknown): { header: string; content: string } {
  ...
  // Legacy string format: try to split on "HEADER: content"
  const str = String(b ?? "")
...
/* ════════════════════════════════════════════════════════
   THINKING VIEW — Premium loading animation
   ════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════
   LISTING COPYWRITER — Main component
   ════════════════════════════════════════════════════════ */
function ListingCopywriter() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Parse bullet points: handle both { header, content } objects and plain strings
  ...
  // Before hydration, show a loading placeholder instead of null
  if (!isClient) {
    return (
  ...
  // ── LOADING ──
  if (isLoading) {
    return <ThinkingView productName={productName} />
  }
  ...
  if (result) {
    ...
    return (
      <div className="mx-auto max-w-4xl flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <div className="flex items-center gap-1 rounded-md bg-success/10 px-2 py-0.5">
                  ...
                  <span className="text-[10px] font-bold text-success uppercase tracking-wide">Optimized for Amazon A9 Algorithm</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {(result?.bulletPoints?.length ?? 0) > 0 && (
          <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              ...
              <CopyButton
                text={(result?.bulletPoints ?? []).map((b) => b?.header ? `${b.header}: ${b.content}` : b?.content ?? "").join("\n")}
                label="Copy All"
              />
            </div>
            <div className="flex flex-col divide-y divide-border/50">
              {(result?.bulletPoints ?? []).map((bullet, i) => {
                const displayText = bullet?.header ? `${bullet.header}: ${bullet.content}` : (bullet?.content ?? "")

                return (
                  <div key={i} className="flex items-start gap-4 px-6 py-4 hover:bg-secondary/30 transition-colors group">
                    <div className="mt-1 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-success/10 border border-success/20">
                      <Check className="h-4 w-4 text-success" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <span className="text-sm text-foreground leading-relaxed">
                        {bullet?.header && (
                          <strong className="font-bold text-foreground">{bullet.header}: </strong>
                        )}
                        {bullet?.content ?? ""}
                      </span>
                    </div>
        ...
        {/* Market Strategy -- SEO keyword highlights */}
          ...
          <div className="rounded-2xl border border-primary/20 bg-primary/5 shadow-sm overflow-hidden">
            ...
            <div className="px-6 py-5">
              <div className="flex flex-wrap gap-2">
                {/* Extract meaningful keywords from title and bullet headers */}
                {[
                  ...
                  .map((keyword, i) => (
                    <span
                      key={i}
                      className="rounded-full border border-primary/20 bg-card px-3 py-1 text-xs font-medium text-foreground"
                    >
                      {keyword}
                    </span>
                  ))}
              ...
              <div className="mt-4 flex items-center gap-4 text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-1.5">
                  <div className="h-2 w-2 rounded-full bg-success" />
                  <span>{result?.bulletPoints?.length ?? 0} bullets</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="h-2 w-2 rounded-full bg-success" />
                  <span>A9 indexed</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Product Description -- rendered as rich HTML */}
        {result?.description && (
          <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              <div className="flex items-center gap-3">
              ...
              {descriptionHasHtml ? (
                <div
                  ...
                  className="prose prose-sm max-w-none text-foreground
                    ...
                    [&_ul]:list-disc [&_ul]:ml-5 [&_ul]:mb-3 [&_ul]:flex [&_ul]:flex-col [&_ul]:gap-1.5
                    [&_ol]:list-decimal [&_ol]:ml-5 [&_ol]:mb-3 [&_ol]:flex [&_ol]:flex-col [&_ol]:gap-1.5
                    [&_li]:text-sm [&_li]:leading-relaxed [&_li]:text-foreground
                    [&_h2]:text-base [&_h2]:font-bold [&_h2]:mt-4 [&_h2]:mb-2 [&_h2]:text-foreground
                    [&_h3]:text-sm [&_h3]:font-bold [&_h3]:mt-3 [&_h3]:mb-1.5 [&_h3]:text-foreground
                    [&_br]:block [&_br]:mb-1"
          ...
          <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            ...
            <div className="px-6 py-5">
              ...
              <span className="mt-2 block text-[10px] text-muted-foreground/60">
                Amazon allows up to 249 bytes for backend search terms.
              </span>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-6 p-6">
          ...
          <div>
            ...
            <span className="mt-1.5 block text-[10px] text-muted-foreground/50">
              Your product name as it should appear on Amazon.
            </span>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={"Describe the key features, materials, and benefits.\n\ne.g. Double-wall vacuum insulation, keeps drinks cold 24hrs, BPA-free, leak-proof lid, fits standard cup holders, comes in 6 colors..."}
              rows={7}
            ...
            />
            <span className="mt-1.5 block text-[10px] text-muted-foreground/50">
              List features, materials, dimensions, unique selling points, and target audience.
            </span>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder='e.g. "AquaPure", "ThermoFlex"'
              className="w-full rounded-xl border border-border bg-secondary/20 px-4 py-3.5 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 placeholder:text-muted-foreground/40 transition-all"
            />
            <span className="mt-1.5 block text-[10px] text-muted-foreground/50">
              If left blank, a [YOUR_BRAND] placeholder will be used in the listing.
            </span>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder='e.g. "water bottle, insulated bottle, gym bottle, BPA free"'
              className="w-full rounded-xl border border-border bg-secondary/20 px-4 py-3.5 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 placeholder:text-muted-foreground/40 transition-all"
            />
            <span className="mt-1.5 block text-[10px] text-muted-foreground/50">
              Comma-separated keywords you want to rank for on Amazon.
            </span>
          </div>
        </div>

        {/* Error message -- inline red text above button */}
        {error && (
          ...
          {!canGenerate && (
            <div className="mb-3 text-center text-xs text-muted-foreground">
              {!productName.trim()
                ...
                : "Add target keywords to enable generation"}
            </div>
          )}
          <Button
            onClick={handleGenerate}
            ...
            className={cn(
              "w-full rounded-xl px-6 py-3.5 text-sm font-bold shadow-lg transition-all",
              canGenerate
